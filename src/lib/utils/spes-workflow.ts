...
export function toApiPriority(
  priority: PrismaPriorityLevel | null,
...
export function toApiSelectionStatus(
  status: PrismaSelectionStatus,
): SpesSelectionStatus {
  return status.toLowerCase() as SpesSelectionStatus;
}

export function toApiApplicantCategory(
  applicantType: PrismaApplicantType,
): SpesApplicantCategory {
  return applicantType === "SPES_BABY" ? "spes_baby" : "new";
}

export function toDbSelectionStatus(
...
const PRIORITY_POINTS: Record<ApplicantPriority, number> = {
  high: 90,
  moderate: 60,
  low: 30,
};

function toPriorityPoints(
  priority: PrismaPriorityLevel | ApplicantPriority | null,
): number {
  if (!priority) return 0;
  const normalized = priority.toLowerCase() as ApplicantPriority;
  return PRIORITY_POINTS[normalized] ?? 0;
}
...
export function computeWorkflowRankMap(
  ...
  const totalScore =
    options?.totalScore && options.totalScore > 0 ? options.totalScore : 100;
  const highestPriority = 90;
  ...
  const rankedEntries = workflows
    .filter(
      (
        ...
        workflow.applicantCategory !== "spes_baby" &&
        workflow.examScore !== null,
    )
    .map((workflow) => {
      const scoreComponent = (workflow.examScore / totalScore) * 0.4;
      const priorityComponent =
        (toPriorityPoints(workflow.priority) / highestPriority) * 0.6;
      const compositeScore = (scoreComponent + priorityComponent) * 100;
      return {
        ...workflow,
        compositeScore,
      };
...
export function toWorkflowListItem(
  ...
  return {
    ...
    applicantCategory: toApiApplicantCategory(
      workflow.submission.applicantType,
    ),
    ...
    remarks: workflow.submission.profile.spes?.remarks || null,
